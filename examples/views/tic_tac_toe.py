...
class TicTacToeButton(discordtool.ui.Button["TicTacToe"]):
    def __init__(self, x: int, y: int):
        # A label is required, but we don't need one so a zero-width space is used.
        # The row parameter tells the View which row to place the button under.
        # A View can only contain up to 5 rows -- each row can only have 5 buttons.
        # Since a Tic Tac Toe grid is 3x3 that means we have 3 rows and 3 columns.
        super().__init__(style=discordtool.ButtonStyle.secondary, label="\u200b", row=y)
        self.x = x
        self.y = y
    ...
    async def callback(self, interaction: discordtool.Interaction):
        ...
        if view.current_player == view.X:
            self.style = discordtool.ButtonStyle.danger
            self.label = "X"
            view.board[self.y][self.x] = view.X
            view.current_player = view.O
            content = "It is now O's turn"
        else:
            self.style = discordtool.ButtonStyle.success
            self.label = "O"
...
class TicTacToe(discordtool.ui.View):
