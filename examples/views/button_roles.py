...
class RoleButton(discordtool.ui.Button):
    def __init__(self, role: discordtool.Role):
        """A button for one role. `custom_id` is needed for persistent views."""
        super().__init__(
            label=role.name,
            style=discordtool.ButtonStyle.primary,
...
class ButtonRoleCog(commands.Cog):
    ...
    async def post(self, ctx: discordtool.ApplicationContext):
        """Slash command to post a new view with a button for each role."""

        # timeout is None because we want this view to be persistent.
        view = discordtool.ui.View(timeout=None)
