...
async def global_command(
    ctx: discordtool.ApplicationContext, num: int
):  # Takes one integer parameter
    await ctx.respond(f"This is a global command, {num}!")


@bot.slash_command(guild_ids=[...])
async def joined(ctx: discordtool.ApplicationContext, member: discordtool.Member = None):
    # Setting a default value for the member parameter makes it optional ^
    user = member or ctx.author
    await ctx.respond(
        f"{user.name} joined at {discordtool.utils.format_dt(user.joined_at)}"
    )
